...
if __name__ == '__main__':
    ...
    wandb.init(project="mlmodels", entity="brain-health",
              settings=wandb.Settings(_disable_stats=True), mode='dryrun')
    
    
    accumulations = {0:8, 4:4, 8:1}
    
    if CONFIG['ACCUMULATE_GRAD_BATCHES']:
        accumulator = GradientAccumulationScheduler(scheduling=accumulations)
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[StochasticWeightAveraging(swa_lrs=1e-2)], #ImagePredictionLogger(samples)], accumulator, early_stopping
        #gradient_clip_val=1,
        #auto_scale_batch_size = AUTO_BATCH_SIZE,
        auto_lr_find=CONFIG['AUTO_LEARNING_RATE']
    )
